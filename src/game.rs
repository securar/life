...
const BG_COLOR: Color = Color::new(0.04, 0.04, 0.04, 1.00); // almost black
const HOVERED_COLOR: Color = DARKGRAY;
const ALIVE_COLOR: Color = Color::new(0.95, 0.95, 0.95, 1.00); // almost white

const MIN_ZOOM_X: f32 = 0.00002;
const MAX_ZOOM_X: f32 = 1.0;
const DEFAULT_ZOOM_X: f32 = 0.03;
// the amount the camera will move using the keyboard
const KB_MOVE_AMOUNT: f32 = 0.05;

const TEXT_SIDE_PAD: f32 = 10.0;

const FPS_CLOCK_INTERVAL: f64 = 0.25;
const GRID_CLOCK_INTERVAL: f64 = 0.01;
...
impl Game {
    pub fn new(is_fullscreen: bool, state: GameState) -> Self {
        let camera = Camera2D {
    ...
    fn get_frustum(&self) -> Rect {
        ...
        let size = (top_left - bottom_right).abs();

        // create a frustum rectangle
        return Rect::new(top_left.x, top_left.y, size.x, size.y);
    }

    fn get_cell_rect(&self, pos: &CellPos) -> Rect {
        let pos = Vec2::new(pos.x as f32, pos.y as f32);
        Rect::new(pos.x, pos.y, 1.0, 1.0)
    }
    ...
    fn save_state(&self) {
        GameState {
            ...
            camera_zoom_x: self.camera.zoom.x,
            camera_zoom_y: self.camera.zoom.y,
            camera_target_x: self.camera.target.x,
            camera_target_y: self.camera.target.y,
        }
        .save();
    }
    ...
    pub fn handle_controls(&mut self) {
        ...
        if is_key_down(KeyCode::LeftShift) && is_mouse_button_down(MouseButton::Right) {
